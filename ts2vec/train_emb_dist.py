...
if __name__ == '__main__':
    ...
    parser.add_argument('--mtsfile', help='The mts name')
    parser.add_argument('--simfile', help='The file for output sims')
    ##########################################################################################
    ...
    parser.add_argument('--loader', type=str, required=True,
                        help='The data loader used to load the experimental data. '
                             'This can be set to UCR, UEA, forecast_csv, forecast_csv_univar, '
                             'anomaly, anomaly_coldstart, EMB_ALL OR EMB_EACH')
    parser.add_argument('--dist_func', type=str, required=True,
                        help='Distance function for similarity computing: euclidean OR cosine')
    ##########################################################################################
    ...
    dirstr, ext = os.path.splitext(args.mtsfile)
    dataset = dirstr.split('/')[-1]
    ...
    if args.loader == 'EMB_ALL':
        task_type = 'embedding_all'
        data = datautils.load_sim_all(args.mtsfile)

    elif args.loader == 'EMB_EACH':
        task_type = 'embedding_each'
        data = datautils.load_sim_each(args.mtsfile)
    else:
        raise ValueError(f"Unknown loader {args.loader}.")
    ##########################################################################################
    ...
    run_dir = 'training/' + dataset + '__' + name_with_datetime(args.run_name)
    ...
    ##########################################################################################
    if args.eval:
        if task_type == 'embedding_all':
            t = time.time()
            input_dims = data[0].shape[-1]
            ...
            t = time.time() - t
            print(f"\nTraining time: {datetime.timedelta(seconds=t)}\n")
            reprs = model.encode(data, encoding_window='full_series')
            sims = tasks.compute_sims_all_dist(reprs, args.dist_func)

        elif task_type == 'embedding_each':
            input_dims = 1
            model = TS2Vec(
                input_dims,
                device=device,
            ...
            for ix in data_keys:
                ...
                t = time.time() - t
                print(f"\nTraining time: {datetime.timedelta(seconds=t)}\n")
                reprs[ix] = model.encode(data[ix], encoding_window='full_series')
            
            sims = tasks.compute_sims_each_dist(reprs, args.dist_func)
        else:
            assert False

        np.save(args.simfile, sims)
