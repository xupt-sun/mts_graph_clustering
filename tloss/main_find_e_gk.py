...
from main_cluster3 import normalize_sims
...
def parse_args():
    ...
    parser.add_argument('--simfile_prefix', nargs='?', required=True,
                        help='Prefix of sim file.')
    ...
    parser.add_argument('--run_times', type=int, default=20, 
                        help='Max ID of sim file ')
...
def main():
    ...
    if args.combine_flag == 0 or args.combine_flag == 2:
        all_eks = np.zeros((args.run_times, 1, 6))
    else:
        print('Error combine_flag, should be: 0, 2')
        exit(-1)

    for id in range(1, args.run_times+1):
        sim_file = args.simfile_prefix + '.' + str(id) + '.npy'
        sim_scores = np.load(sim_file)
        sim_scores_norm = normalize_sims(sim_scores)
        eks = find_e(mts_data, sim_scores_norm, args.combine_flag, args.k, args.max_min_comp)
        all_eks[id-1] = eks

    np.save(args.output, all_eks)
